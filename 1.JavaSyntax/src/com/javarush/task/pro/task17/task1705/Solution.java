...
public class Solution {
    ...
    public static void runWorkingProcess() {
        ...
        for (int a=0;a<astronauts.size();a++) {
            if (astronauts.get(a) instanceof Human) {
                //Human human1 = (Human) astronauts.get(a);
                pilot((Human) astronauts.get(a));
            }
        }
        for (int a=0;a<astronauts.size();a++) {
            if (astronauts.get(a) instanceof Dog) {
                Dog dog = (Dog) astronauts.get(a);
                createDirection(dog);

            }
        }
        for (int a=0;a<astronauts.size();a++) {
            if (astronauts.get(a) instanceof Cat) {
                Cat cat = (Cat) astronauts.get(a);
                research(cat);
